...
main :-
    ...
    write('Введите размер поля N (например, 3): '), read(N),
    write('Введите длину последовательности для победы K (например, 3): '), read(K),
...
% --- ввод хода ---
read_move(Row, Col) :-
    write('Введите номер строки: '), read(Row),
    write('Введите номер столбца: '), read(Col).
...
print_board(Board) :-
    ...
    atom_length(AtomN, MaxIdxLen),   % Длина самого длинного индекса (высота)
    ColWidth is MaxIdxLen + 2,       % Ширина столбца = длина индекса + 2
    Indent is MaxIdxLen + 1,         % Отступ слева для заголовков (индекс строки + 1 пробел)

    % 2. Печать заголовков столбцов
...
print_header(Current, N, _, _) :- Current > N, nl, !.
print_header(1, N, ColWidth, Indent) :-
    % Сначала печатаем отступ слева
    tab(Indent+1),
    % Печатаем первый пробел (над первой вертикальной чертой)
    write(' '),
    % Печатаем число
    print_centered(1, ColWidth),
    Next is 1 + 1,
    print_header(Next, N, ColWidth, Indent).
print_header(Current, N, ColWidth, Indent) :-
    Current > 1,
    write(' '), % Пробел над разделителем столбцов
    print_centered(Current, ColWidth),
    Next is Current + 1,
    print_header(Next, N, ColWidth, Indent).

% --- Печать горизонтального разделителя (+---+---+) ---
print_separator(N, ColWidth, BaseIndent) :-
    % Добавляем 2 к базовому отступу для сдвига вправо на 2 символа.
    NewIndent is BaseIndent + 2,
    tab(NewIndent),              
    write('+'),                  % Первое перекрестие
...
print_rows(Board, StartIdx, MaxIdxLen, ColWidth) :-
    ...
    % Печатаем: Индекс + Данные -> Разделитель
    print_rows_loop(Board, StartIdx, MaxIdxLen, ColWidth, N, Indent),
    
    % ИСПРАВЛЕНИЕ: Вызываем разделитель, закрывающий низ поля.
    print_separator(N, ColWidth, Indent).
...
print_rows_loop([Row|Rs], RowNum, MaxIdxLen, ColWidth, N, Indent) :-
    ...
    format('~*d |', [MaxIdxLen, RowNum]), % Индекс + пробел + палка (вертикальная грань)
...
print_row_cells([Cell|Cs], ColWidth) :-
    print_centered(Cell, ColWidth),
    write('|'),
    print_row_cells(Cs, ColWidth).

% --- Универсальное центрирование (для чисел и атомов x/o) ---
print_centered(Content, Width) :-
    ( integer(Content) -> term_to_atom(Content, A) ; A = Content ),
    atom_length(A, Len),
    PadL is Width // 2,
    PadR is Width - PadL - Len,
    ( PadR < 0 -> PadRFinal = 0 ; PadRFinal = PadR ),
    format('~*c~w~*c', [PadL, 32, A, PadRFinal, 32]). % 32 - код пробела
