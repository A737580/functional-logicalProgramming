% tictactoe.pl готовое форматирование поля.
...
print_board(Board) :-
    ...
    Indent is MaxIdxLen + 1,         % Отступ слева

    % 2. Печать заголовков столбцов
    print_header(1, N_Int, ColWidth, Indent+1),
...
print_header(1, N, ColWidth, Indent) :-
    tab(Indent),
    % ВОССТАНОВЛЕННОЕ ИСПРАВЛЕНИЕ: Сдвиг вправо на 1 символ
    write(' '), 
    % Печатаем число (оно будет преобразовано в целое внутри print_centered)
    print_centered(1, ColWidth),
    Next is 1 + 1,
    print_header(Next, N, ColWidth, Indent).
print_header(Current, N, ColWidth, Indent) :-
    Current > 1,
    % ВОССТАНОВЛЕННОЕ ИСПРАВЛЕНИЕ: Один пробел для уменьшения отступа
...
print_rows_loop([Row|Rs], RowNum, MaxIdxLen, ColWidth, N, Indent) :-
    ...
    % АГРЕССИВНОЕ ИСПРАВЛЕНИЕ ТОЧЕК: Ручное форматирование
    RNum_Int is round(RowNum),
    term_to_atom(RNum_Int, RNum_Atom),
    atom_length(RNum_Atom, RNum_Len),
    
    % Вычисляем отступ для выравнивания по правому краю
