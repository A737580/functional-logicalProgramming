...
main :-
    ...
    ( validate_settings(N, K) ->
        ...
        writeln('1. Человек против Человека'),
        writeln('2. Человек против Робота'),
        read_number('Ваш выбор (1 или 2): ', ModeNum),
        
        ( ModeNum = 2 -> Mode = pva ; Mode = pvp ),
        
        create_empty_board(N, Board),
        
        format('~nНачинаем игру! Режим: ~w. Вы играете за X.~n', [Mode]),
        play(Board, N, K, x, Mode)
    ;
...
% Логика получения хода в зависимости от того, кто ходит
get_turn_move(Board, N, K, Player, Mode, Row, Col) :-
    % Если режим PvA (робот) и сейчас ходит 'o' (робот)
    Mode == pva, Player == o, !,
    writeln('Робот думает...'),
    % Вызываем логику ИИ
    choose_ai_move(Board, N, K, Player, Row, Col),
    format('Робот сходил в строку ~w, столбец ~w~n', [Row, Col]).

get_turn_move(_, _, _, Player, _, Row, Col) :-
    % Во всех остальных случаях спрашиваем человека
    ask_move(Row, Col, Player).


% Логика окончания игры
check_game_over(Board, _, K, Player, _) :-
    check_winner(Board, K, Player), !, % Если есть победитель
    print_board(Board),
    format('>>> ИГРОК ~w ПОБЕДИЛ! ПОЗДРАВЛЯЕМ!~n', [Player]).

check_game_over(Board, _, _, _, _) :-
    is_board_full(Board), !,           % Если доска полная
    print_board(Board),
    writeln('>>> НИЧЬЯ! Больше нет ходов.').

check_game_over(Board, N, K, Player, Mode) :-
    % Если игра не окончена, меняем игрока и продолжаем
    switch_player(Player, NextPlayer),
    play(Board, N, K, NextPlayer, Mode).
...
% Стратегия выбора хода (приоритеты):
% 1. Победить сейчас.
% 2. Блокировать победу соперника.
% 3. Занять центр (если поле нечетное и центр свободен).
...
choose_ai_move(Board, N, _, _, Row, Col) :-
...
% --- 1. Попытка победить ---
% Ищем пустую клетку, ставим туда свой знак, проверяем на победу.
try_win_move(Board, N, K, Player, Row, Col) :-
    all_valid_moves(Board, N, Moves),
    member(Row-Col, Moves), % Перебираем возможные ходы
    make_move(Board, Row, Col, Player, TestBoard),
    check_winner(TestBoard, K, Player).

% --- 2. Попытка блокировать ---
% То же самое, но проверяем, выиграет ли враг, если сходит туда.
try_block_move(Board, N, K, Enemy, Row, Col) :-
    all_valid_moves(Board, N, Moves),
...
% (Код отрисовки полностью сохранен)
% ==========================================
...
print_centered(Content, Width) :-
    ( number(Content) -> Num is round(Content), term_to_atom(Num, Atom) ; Atom = Content ),
    atom_length(Atom, Len),
    PadLeft is Width // 2,
