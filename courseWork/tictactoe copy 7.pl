% =================================================================================
% TIC-TAC-TOE (Крестики-нолики) с AI есть два режима игры с II, но ии не умеет играть за Х и делает всегда рандомные ходы на большом поле
...
main :-
    ...
    ( validate_settings(N, K) ->
        ...
        setup_game_mode(ModeNum, Mode, HumanSide), % Настройка режима и стороны
...
setup_game_mode(1, pvp, both) :- !. % В PvP оба человека, 'both' - заглушка
...
play(Board, N, K, Player, Mode, HumanSide) :-
    ...
    ( is_valid_move(Board, Row, Col) ->
        ...
        check_game_over(NewBoard, N, K, Player, Mode, HumanSide) % Обновлено
    ;
        % Если ход неверен (только для человека):
        writeln('>>> Неверный ход (клетка занята или вне поля). Попробуйте снова.'),
        play(Board, N, K, Player, Mode, HumanSide) % Обновлено
    ).

% Логика получения хода в зависимости от того, кто ходит
% Случай 1: Режим с Роботом, и сейчас ход НЕ человека.
get_turn_move(Board, N, K, Player, pva, HumanSide, Row, Col) :-
    Player \== HumanSide, !, % Робот ходит, если Player не совпадает с HumanSide
...
% Случай 2: Все остальные (Ход человека в PvA или любой ход в PvP)
get_turn_move(_, _, _, Player, _, _, Row, Col) :-
    ask_move(Row, Col, Player).


% Логика окончания игры (Обновлено: добавлен HumanSide)
...
check_game_over(Board, N, K, Player, Mode, HumanSide) :- % Обновлено
    % Если игра не окончена, меняем игрока и продолжаем
    switch_player(Player, NextPlayer),
    play(Board, N, K, NextPlayer, Mode, HumanSide). % Обновлено
...
% (С логикой индексов, исправленной в прошлых шагах)
% ==========================================
...
print_board(Board) :-
    ...
    print_separator(N, ColWidth, Indent), % ЗАКРЫВАЮЩИЙ РАЗДЕЛИТЕЛЬ
    nl.
