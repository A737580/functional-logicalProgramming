% =================================================================================
% TIC-TAC-TOE (Крестики-нолики) с AI  Конечная вариация игры, можно докручивать игру робота за Х.
...
choose_ai_move_first(Board, N, _, _, Row, Col) :-
    % 3. Если это первый ход (и доска пуста): ВСЕГДА БРАТЬ ЦЕНТР
    is_board_empty(Board),
    try_center_move(Board, N, Row, Col), !.

choose_ai_move_first(Board, N, _, _, Row, Col) :-
    % 4. Если доска пуста, но центр не берется (например, K=N=2): БРАТЬ УГОЛ
    is_board_empty(Board),
    try_corner_move(Board, N, Row, Col), !.

choose_ai_move_first(Board, N, K, Player, Row, Col) :-
    % 5. Агрессивный ход (создание вилок, углы/края)
    % На этом шаге можно добавить более сложную логику вилок, но пока
    % оставим выбор из углов и краев для развития угрозы.
...
% --- СТРАТЕГИЯ ИИ: ХОДИТ ВТОРЫМ (o) --- (Защитная и реактивная)
choose_ai_move_second(Board, N, K, Player, Row, Col) :-
    % 1. Победить сейчас
    try_win_move(Board, N, K, Player, Row, Col), !.
...
is_board_empty(Board) :-
    % Проверяет, что на доске нет X или O
    \+ (member(Row, Board), member(Cell, Row), Cell \= ' ').

% АГРЕССИВНЫЙ ХОД (Для 'x', если центр уже не первый ход)
% Ищет ход, который является углом или краем, чтобы избежать "ленивого" центра
try_aggressive_move(Board, N, _, _, Row, Col) :-
    % 1. Находим все валидные ходы
    all_valid_moves(Board, N, Moves),
    
    % 2. Отделяем ходы в углы и по краям (не центр)
    findall(R-C, 
            ( member(R-C, Moves),
              \+ is_center(R, C, N) % Исключаем центр
            ), 
            NonCenterMoves),
    
    ( NonCenterMoves = [] ->
        % Если остались только центральные ходы, берем любой
        Moves = [Row-Col|_] 
    ; 
        % Иначе, берем случайный ход из углов/краев
        random_member(Row-Col, NonCenterMoves)
    ).

% --- Новый предикат: Попытка занять УГОЛ ---
try_corner_move(Board, N, Row, Col) :-
    N_Int is round(N),
    N_Int > 1, % Углы имеют смысл только на полях >= 2x2
    
    % Координаты углов: (1,1), (1,N), (N,1), (N,N)
    Corners = [1-1, 1-N_Int, N_Int-1, N_Int-N_Int],
    
    member(Row-Col, Corners),
    is_valid_move(Board, Row, Col).
